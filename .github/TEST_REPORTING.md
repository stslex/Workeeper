...
- ✅ Pass/fail statistics with detailed counts
...
- Check "🧪 UI Test Results" in the Checks tab
...
- ✅ Detailed pass/fail/skip counts per test
- 🎯 Test execution time per individual test
- 📋 Test suite breakdown by module
- ⚠️ Failure messages and full stack traces
- 📊 **Rich job summary** in workflow run page
...
- Check "📊 Detailed Test Report" in the Checks tab
...
- 📍 **Code annotations** for failed tests (shows exactly where tests are)
- 🔍 Filterable and searchable test results
- ⏱️ Performance metrics per test
- 📊 Pass rate visualization
- 🎯 Test grouping by class/suite
...
- Check "📈 Test Report" in the Checks tab
...
1. **Check Status** - ✅/❌ status for each API level tested in the PR checks section
2. **Test Summary Comment** - Auto-generated by EnricoMi action with:
    - Total tests run
    - Pass/fail/skip statistics
    - Pass rate percentage
    - Comparison with previous run (trend ↗️/↘️)
    - Links to check runs for details
...
Results for commit abc1234. ± Comparison against earlier commit def5678.
...
- 🧪 UI Test Results (API 34) - ✅ Passed
- 📊 Detailed Test Report (API 34) - ✅ Passed
- 📈 Test Report (API 34) - ✅ Passed
...
|----------------------------------|--------------------------------|-----------|-------------------|
| 📊 `ui-test-reports-api-{level}` | HTML test reports, XML results | 30 days   | Full test reports |
| 📸 `screenshots-api-{level}`     | Screenshots of failed UI tests | 14 days   | Only on failures  |
| 📝 `logcat-api-{level}`          | Android logcat output          | 7 days    | Debugging info    |
...
- Trend comparison (↗️ more tests, ↘️ fewer tests)
- Links to detailed checks

**Step 3: Click on checks for details**

- "🧪 UI Test Results" - Overall summary
- "📊 Detailed Test Report" - Per-test breakdown
- "📈 Test Report" - Visual report with annotations
...
| 90-99%    | ⚠️ Good            | Review failed tests          |
| 80-89%    | ⚠️ Needs attention | Fix failing tests            |
...
- ✅ Verify tests ran: Check workflow logs
- ✅ Check XML files: Look in `build/outputs/androidTest-results/`
- ✅ Ensure tests not all `@Ignore`d
