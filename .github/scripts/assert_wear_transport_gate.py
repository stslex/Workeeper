...
1. No tracked Kotlin source may contain the Data Layer package name at all. Text
...
2. No tracked Kotlin source may suppress the gate, by rule id, by rule-set id, by
...
# Matched on a package boundary, so `...gms.wearablefake` is not a hit. Only `.kt`/`.kts` are
...
def tracked_kotlin_files() -> list[Path]:
