...
internal object AppGraphSourceModule {
    ...
    fun provideAppGraph(
        ...
        // App-Scope Collapse Step 5 (5a). The two DB-cascade `create()` roots the graph bridge-reads,
        // injected FROM HILT — so under @TestInstallIn (TestInfraModule swaps FakeImageStorage) the test-branch
        // build passes the FAKE ImageStorage into create(), never a real one; and the in-memory AppDatabase
        // (from the test-DB module) flows in as the appDatabase root. The 9 DAOs + DbTransitionRunner now DERIVE
        // from appDatabase graph-internally (DbCascadeBindingContainer). Fake-awareness guarantee: bridge-READ,
        // never construct. No dagger.Lazy needed — no @IO→appGraph back-edge (derived bindings read AppDatabase /
        // a direct Dispatchers.IO).
        ...
        // adopt-back shims resolve, and the bridged roots (incl. FakeImageStorage + in-memory AppDatabase) come
        // from Hilt.
        else -> buildAppGraph(
