...
internal class SnackbarManagerTest {
    ...
    /**
     * The epoch is a `MutableStateFlow` advanced through `update`'s CAS loop. This pins the
     * linearizability of that choice under real parallelism: a `value = value + 1` replacement
     * passes every other test in this suite and loses increments only here.
     */
    @Test
    fun `concurrent committed handovers lose no epoch increment`() = runTest {
        drainLeftovers()
        val before = deliverOnce(AppSnackbarModel(message = "epoch-before")).epoch
        ...
        withContext(Dispatchers.Default) {
            repeat(workers) {
                launch {
                    repeat(perWorker) { SnackbarManager.advanceGenerationEpoch() }
                }
            }
        }

        val after = deliverOnce(AppSnackbarModel(message = "epoch-after")).epoch
        assertEquals(before + workers * perWorker, after)
    }
