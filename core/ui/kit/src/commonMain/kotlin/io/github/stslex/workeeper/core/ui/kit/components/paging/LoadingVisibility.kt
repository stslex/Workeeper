...
private fun rememberLoadingVisible(loading: Boolean): Boolean {
    ...
    LaunchedEffect(loading) {
        when (val step = loadingStep(loading, visible, shownAt, monotonicNowMillis(), motion)) {
            is LoadingStep.ShowAfter -> {
                delay(step.delayMillis)
                shownAt = monotonicNowMillis()
                visible = true
            }
 ...
 *
 * GUARD: feed this from [monotonicNowMillis]; a wall-clock adjustment would land directly in
 * the subtraction.
 */
...
/**
 * Origin of the module's monotonic millisecond clock. GUARD: monotonic, never wall-clock —
 * [loadingHoldRemaining] subtracts two of these readings.
 */
private val loadingClockOrigin = TimeSource.Monotonic.markNow()

/** Milliseconds since [loadingClockOrigin]; only ever compared against itself. */
internal fun monotonicNowMillis(): Long = loadingClockOrigin.elapsedNow().inWholeMilliseconds
