...
fun AppTheme(
    ...
    PlatformWindowChrome(darkTheme = darkTheme)
...
/**
 * Host/window side effect behind [AppTheme]: Android owns the status-bar appearance here, while
 * the future iOS host owns native window chrome, so the iOS implementation is a no-op.
 */
@Composable
internal expect fun PlatformWindowChrome(darkTheme: Boolean)
