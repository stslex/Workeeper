...
| Seam | Decision | Why |
|---|---|---|
| `PlatformInfoProvider` | **expect/actual class** | Pure platform-value provider (version name/code, device model). The iOS actual is trivially implementable now (`NSBundle` info dictionary, `UIDevice.model`) and is included compile-verified. Known granularity gap on `deviceModel()`: Android's bare `Build.MODEL` is not strictly equivalent to iOS `UIDevice.model` — adequate for crash/diagnostics context, normalisation revisited when the iOS value is first consumed. DI simplifies: the `@ContributesBinding` interface binding is replaced by injecting the class (`@SingleIn(AppScope)` + `@Inject` on the Android actual's constructor). |
| `AppReinitializer` | **expect/actual class** | Single platform primitive. Android actual keeps the launch-intent + `Runtime.getRuntime().exit(0)` restart verbatim. The iOS actual is a loud `TODO()` — Apple rejects `exit()`, so the likely iOS shape is an **in-process graph rebuild**; that redesign belongs to Phase 5. **As measured at Phase 3** the reason it was not trivial: three `@SingleIn(AppScope)` classes (`BackupPreferencesRepositoryImpl`, `RestoreStateRepositoryImpl`, `AppDialogRepository`) bypassed `DataStoreProvider`'s memoization and threw on a second graph — a throw swallowed by `AppCoroutineScopeImpl.launch`'s `.catch`, so the symptom was missing data, not a crash. **Phase 6 closed the bypass** (`kmp-phase-6-data-layer.md` §1), so the expect KDoc no longer records it; it carries the forward invariant instead — every app-scoped `DataStore` holder mints through the provider — pinned by `app/app` androidTest `AppScopeDataStoreSingletonTest`. The `.catch` swallow is untouched and still open: `documentation/tech-debt.md` → "The default flow `onError` is silent". |
...
| `TempFileProvider` | **not converted** — interface + impl + binding move to androidMain, Android-only | `java.io.File` does not exist on Kotlin/Native, and the seam is already slated for **removal**: it is a tactical decoupling so the domain layer stops importing `Context.cacheDir` while temp-file orchestration still lives inside interactors. When that orchestration moves to the data layer the interface goes away — it is explicitly not to be reshaped into a neutral path abstraction. Converting would contradict that decision. |
...
- androidMain declares `api(libs.javax.inject)`, not `implementation`: the four dispatcher
  qualifier annotations are public types carrying `@javax.inject.Qualifier`, and downstream
  modules must see that meta-annotation or Metro cannot recognise the qualifier — the app-scope
  aggregation then drops the qualified dispatchers. It arrived transitively via `hilt.android`
  before; after Hilt's removal it is the bare artifact declared here.
- The Firebase BOM is applied as `"androidMainImplementation"(platform(libs.google.firebase.bom))`
  in the top-level `dependencies { }` block, using the raw configuration name, because the KMP
  source-set DSL's `platform()` is deprecated/removed in Kotlin 2.3 — it cannot be written inside
