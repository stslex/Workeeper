...
```
Implement Stage 5.4 — Live workout feature + minimal Home shortcut — per documentation/feature-specs/live-workout.md.

CONTEXT
This is the fourth v1 feature implementation, after Settings + Archive (5.1), Exercises (5.2), Trainings (5.3). It adds the core training execution flow: starting a session from a training, logging sets inline, finishing with PlanUpdateRule applied per exercise.
...
- documentation/architecture.md (especially: Navigation flow, Localization, Back gesture handling, Compose UI conventions, **DataModel hygiene**)
- documentation/feature-specs/exercises.md (reference)
- documentation/feature-specs/trainings.md (reference, especially Plan editor wrapper pattern)
- documentation/feature-specs/live-workout.md (this feature)
- .claude/skills/add-feature.md

THIS IS A SINGLE PASS. Implementation, verification, draft PR opened. No STOP gate.

DATAMODEL AUDIT (mandatory first step)

Before writing any feature code, complete the DataModel audit per documentation/architecture.md "DataModel hygiene":

1. Open core/exercise/.../exercise/model/ExerciseDataModel.kt. Remove these stale fields:
   - trainingUuid: String? (legacy v2, exercises aren't training-bound in v3+)
   - sets: List<SetsDataModel> (sets live in SetEntity via performed_exercise)
   - labels: List<String> (tags come from exercise_tag join, not denormalized here)

2. Open core/exercise/.../exercise/model/SetsDataModel.kt. Change weight from Double to Double?. Update SetEntity.toData() and SetsDataModel.toEntity() to preserve null. Remove the "?: 0.0" coalesce.

3. Audit usages of removed fields. For each, either remove the call or replace with the canonical source (e.g. labels → query exercise_tag join via TagRepository).

4. Audit PerformedExerciseDataModel and SessionDataModel for any phantom fields. Likely clean — confirm.

5. Document in PR description: per DataModel, list removed fields and count of touched callsites in each feature module.

Run ./gradlew assembleDebug after this step. The project must compile after the audit before proceeding to feature implementation. Fix any compilation errors that result from removed fields by removing or rewriting the call sites.

PROCESS

1. Create new modules:
   - feature/live-workout (new feature module, ComposeViewModel + Hilt + standard MVI structure)
   - feature/home (new minimal module, same structure)
   Register both in settings.gradle.kts. Wire dependencies in app/dev and app/store build.gradle.kts.

2. Add Screen.LiveWorkout(sessionUuid: String?, trainingUuid: String?) to core/ui/navigation/Screen.kt. Verify Screen.BottomBar.Home exists or add it. Update app navigation host.

3. Repository extensions:
   - SetRepository: add upsert(performedExerciseUuid, position, weight, reps, type), deleteByPerformedAndPosition, deleteAllForPerformedExercise, hasAnyForPerformed, countByPerformedExercise. Update SetRepositoryImpl.
   - PerformedExerciseRepository: add insertForSession(sessionUuid, exerciseUuids: List<Pair<String, Int>>). Update Impl.
   - SessionDao: add observeActiveSessionWithStats() per spec. Add ActiveSessionWithStatsRow projection.

4. Domain layer:
   - LiveWorkoutInteractor + Impl per spec — startSession, loadSession, observeSession, upsertSet, deleteSet, setSkipped, resetExerciseSets, finishSession (with transactional PlanUpdateRule application), cancelSession, setPlanForExercise, setAdhocPlan.
   - HomeInteractor + Impl — observeActiveSession returning HomeStore.State.ActiveSessionInfo?

5. UI types in feature/live-workout/.../mvi/model/:
   - LiveExerciseUiModel (@Stable) with status (DONE/CURRENT/PENDING/SKIPPED enum), planSets (ImmutableList<PlanSetUiModel>), performedSets (ImmutableList<PlanSetUiModel>), exerciseName, exerciseType (ExerciseTypeUiModel)
   - LiveSetUiModel (@Stable) with weight, reps, type, isDone
   - LiveSessionStateUiModel (@Stable) — alias or wrapper for state
   - ExerciseStatusUiModel (enum) — DONE, CURRENT, PENDING, SKIPPED
   - All UI types ImmutableList for collections, no kotlin List in @Composable signatures.

6. Mapper feature/live-workout/.../mvi/mapper/LiveWorkoutMapper.kt:
   - SessionSnapshot → LiveWorkoutStore.State (initial)
   - PerformedExerciseSnapshot → LiveExerciseUiModel
   - PlanSetDataModel ↔ PlanSetUiModel (reuse from core/ui/plan-editor)
   - Status derivation per spec (CURRENT = lowest position not done/skipped)

7. Implement LiveWorkoutStore per spec — full State, Action, Event surfaces. Handlers: Click, Input, Common, Navigation, PlanEditAction.

   - Plan editor reuses Action.PlanEditAction(AppPlanEditorAction) wrapper from core/ui/plan-editor (Stage 5.3 pattern).
   - TimerTick coroutine in CommonHandler — `while (active) { delay(1000); consume(Action.Common.TimerTick) }`.
   - Set persistence rule: drafts only flush to DB on explicit checkbox tap. Type changes for done sets also persist.

8. Implement HomeStore + HomeScreen + ActiveSessionBanner.

9. Compose UI:
   - LiveWorkoutScreen, LiveWorkoutGraph, LiveWorkoutHeader, LiveExerciseCard (handles all 4 states), LiveSetRow (weighted/weightless layout).
   - Use AppUi tokens, kit components (AppCard, AppDialog, AppConfirmDialog, AppFAB, AppLinearProgressIndicator if exists, AppButton, AppTextField).
   - Row layout invariant from Stage 5.3 applies to active session banner.
   - All UI consumes UI types (PlanSetUiModel etc), not DataModel.

10. Canonical patterns:
    - NavigationHandler with @Inject Navigator (Hilt).
    - Conditional BackHandler via state.interceptBack — true ONLY when plan editor sheet is dirty.
    - Top-bar back arrow always emits Click.OnBackClick.
    - Action.Navigation.* via consume always wrapped in withContext(immediateDispatcher) at call site if invoked from background.

11. Haptics:
    - HapticClick (LIGHT) on most clicks.
    - HapticImpact (MEDIUM) on set checkbox toggle and on destructive confirms (skip, reset, cancel).
    - No haptic on dismiss-style actions.

12. Localization: every user-facing string via stringResource. EN + RU files per spec.

13. Tests per spec.

14. Composable @Preview for every public/internal Composable, both light and dark modes, multiple states for stateful components.

VERIFICATION

- After DataModel audit, ./gradlew assembleDebug passes.
- ./gradlew :feature:live-workout:assembleDebug detekt lintDebug passes.
- ./gradlew :feature:home:assembleDebug detekt lintDebug passes.
- ./gradlew assembleDebug passes (whole project).
- ./gradlew testDebugUnitTest passes.
- App launches.
  - From Trainings tab → tap training → tap Start session → opens Live workout with that training's exercises pre-listed.
  - Set inputs are pre-filled from plan. Tap checkbox → set saved, row marked done.
  - + Add set adds a new row.
  - Overflow → Edit plan opens AppPlanEditor sheet from core/ui/plan-editor.
  - Overflow → Skip exercise + confirmation marks exercise SKIPPED.
  - Overflow → Reset sets + confirmation deletes all sets for exercise.
  - Finish session button → confirmation dialog with stats → confirm applies PlanUpdateRule and persists.
  - Navigate back to Training detail after finish.
  - Back gesture during workout exits silently — session stays IN_PROGRESS.
- Home tab shows banner with active session details. Tap banner resumes Live workout.
- Home tab shows empty state when no active session.
- In RU locale, all strings translated.

CONSTRAINTS

- Phantom shims related to DataModel audit ARE in scope — must be removed.
- Other phantom shims (e.g. AppDimension legacy nested objects) stay.
- LICENSE / SPDX headers: SPDX-License-Identifier: GPL-3.0-only on every new file.
- Do not implement full Home dashboard (Stage 5.5 — recent sessions, picker, stats). Only banner + empty state.
- Do not implement Past session detail or session edit (Stage 5.5 — read-only history view).
- Do not implement PR detection / personal records (v2).

PR

Open a draft PR titled `feat(live-workout): live workout flow + Home active session banner (Stage 5.4)`. Body:
- Lists changed files grouped by module
- DataModel audit summary (which fields removed, callsite counts per feature)
- Brief summary of new feature behavior
