...
By the time 1.3 runs, the diff is the DSL implementation, the host, and the retention wiring. The 26 `navComponentScreen*` call sites have already moved — one by one, each bisectable — during 1.2.
...
Scope is deliberately migration-shaped, not exhaustive. Broad coverage buys little here: the 26 call sites are uniform and the compiler catches them. The risk lives in three places that are neither uniform nor compiler-checked, and that is where the weight goes:
...
Enforcement: `forbiddenImports` scoped to that source set. A custom detekt rule is disproportionate for one directory.
...
Known non-trivial cases to check explicitly: `LiveWorkout` (its KDoc claims at least one training behind it, and `AllTrainingsStore` has a comment about a session with none), `ExerciseImage` and `PlanEditor` (reached from within `Exercise`, not from a list), `ExerciseChart` (picker-gated).
...
- `planEditorSavedAttr` — save from the plan editor, assert the originating screen reflects it;
...
- scroll position on a seeded list, across a detail round-trip;
...
| `BackStackStateRestorationTest` | recreate list state instead of remembering it on return | the scroll test |
...
**C — no `androidx.navigation*` import** anywhere under `app/app/src/androidTest`. Evidence: grep output, empty.
...
1. **Own registration DSL.** `navComponentScreen*` currently extends `NavGraphBuilder`; under Nav3 the receiver becomes `EntryProviderBuilder`. Introduce a project-owned builder so the 26 call sites never name either.
