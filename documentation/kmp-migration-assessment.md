...
1. **Domain layer has real Android leaks** — 8 files across `recovery`/`settings`/`exercise`, 2 storing Context as Singleton fields, one baking `IntentSender` into a domain model. The custom Detekt rules don't catch this class of leak. Changes the "domain is a cheap win" framing.
   ...
   > **DECISION RECORD — A.5 implemented (this finding is now closed and rule-enforced).** All the leaks above were neutralised in A.1–A.3, and `DomainLayerPurityRule` was **extended** to catch `import android.*` under `feature/<X>/domain/` (mappers included — a mapper must be portable) and **activated** across the whole tree in one green commit. `androidx.*` is deliberately exempt (`startsWith("android.")` excludes it — `androidx.paging`/`androidx.datastore` are KMP-portable and legitimately used in domain). Before activating, a method-verified report-only hit list confirmed **zero** `android.*` imports (and zero inline `android.*` FQN) remained in any `domain/` file — activation was verified green, not assumed. **Known limitation:** the rule inspects import directives only, so an inline `android.foo.Bar` FQN with no import is not detected. This gap **self-closes at the KMP split** — `commonMain` physically cannot see `android.*`, so an inline android FQN won't compile there; Detekt FQN detection is only needed for the transitional window (domain in `androidMain` but required portable), covered by review until then. Domain is FQN-clean today, so the gap is inactive. So "the Detekt rules don't catch this class of leak" is no longer true — they do now, for imports.
...
The restart primitive is now a platform-neutral seam: `AppReinitializer { fun reinitialize() }` in `core/core/platform/` (beside `PlatformInfoProvider`/`TempFileProvider`), since the Phase 3 collapse a true `expect class` whose Android actual (`@SingleIn(AppScope)` `@Inject`, in `core:core` androidMain) carries the one remaining process-restart body. Both previously byte-identical bodies are collapsed into it: `RestoreRecoveryCoordinator.restartApp()` now delegates to `appReinitializer.reinitialize()` (dropping its `@ApplicationContext` field and **all** `android.*` imports — this closes the **last A.2 domain leak**, so all `feature/*/domain/` is now `android.*`-free and A.5 is unblocked), and the Settings post-restore path resolves the seam by **constructor injection into `NavigatorEventBus`** (the `@Singleton` `Navigator` impl): `Navigator.restartApp()` calls `appReinitializer.reinitialize()` directly. *(Correction, KMP Phase 5 / PR #252: the seam and the `NavigatorEventBus` wiring are unchanged and still accurate, but **Settings no longer has a post-restore restart path at all** — `AppRuntime` owns that restart under `ReplacementPolicy.RestartProcess` and calls the host-owned `AppReinitializer` itself. `Navigator.restartApp()` survives with no production caller; see `architecture.md` → "Destructive app-restart through the `AppReinitializer` seam" and `tech-debt.md`.)*
...
| 8 | **Hilt → Metro DI swap** — ~400 sites incl. 22 @HiltViewModel, 9 @AssistedInject/8 @AssistedFactory, 3 @EntryPoint interfaces, 4 custom @Qualifier; convention-plugin rewiring + per-module migration + HiltScopeRule replacement | 25 | 38 | 55 | Low — upper-bound driver |
