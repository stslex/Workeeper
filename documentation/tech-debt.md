...
**The two entries below are SEPARATE, unrelated problems** — an earlier version of this section wrongly
grouped them as "both assisted, no Metro mechanic." Corrected per the Metro-1.1.1 assisted discovery
(decompile of the pinned artifact): **Metro 1.1.1 HAS native assisted injection** — `interop { includeDagger() }`
is a first-class Gradle DSL (the same `interop {}` block we already use for `includeJavax()`), and the
compiler carries the full pipeline (`AssistedInjectChecker` FIR, `AssistedFactoryTransformer` IR,
`AssistedFactoryImpl$Dagger` + `$Metro` — a Dagger-interop path). **No Metro bump is needed**, so there is
no Kotlin-2.4.0 upgrade and no cascade into the ~695-LOC custom Detekt rules. Neither blocker is an assisted
*capability gap*:

- **CommonDataStore** is genuinely assisted, but now **migratable** (mechanic found) — a scoped-consumer flip.
- **ImageStorage** is **not assisted at all** — a plain `@Inject` binding blocked only by a test-fake swap.

| Severity | Location | Description |
|---|---|---|
| 🟡 mechanic found | [core/data/dataStore/.../core/DataStoreProviderFactory.kt](../core/data/dataStore/src/main/kotlin/io/github/stslex/workeeper/core/data/dataStore/core/DataStoreProviderFactory.kt) ↔ [CommonDataStoreImpl.kt](../core/data/dataStore/src/main/kotlin/io/github/stslex/workeeper/core/data/dataStore/store/CommonDataStoreImpl.kt) | **`CommonDataStore` — genuinely assisted, now migratable (own execution slice next).** `CommonDataStoreImpl` (`@Singleton @Inject`) constructor-injects `DataStoreProviderFactory` (Dagger `@AssistedFactory`, `fun create(name: String): DataStoreProvider`) and calls `storeFactory.create(NAME)`; the produced `DataStoreProvider` is `@AssistedInject` with one `@Assisted name: String` + one non-assisted `@ApplicationContext Context`. **Not a hard Step-6 blocker:** the produced `DataStoreProvider` is already **unscoped** (only manually cached by name), so Metro's "assisted injected types cannot be scoped" constraint does not bite — the consumer `CommonDataStoreImpl` stays `@SingleIn(AppScope)`, the factory stays unscoped, matching the current shape. **Migration path:** (a) Metro native assisted — enable `interop { includeDagger() }` so Metro recognizes the existing `dagger.assisted.*` (or convert to `dev.zacsweers.metro.*`), keeping the factory + provider; flip `CommonDataStoreImpl` to `@ContributesBinding(AppScope)`. No bump. Fallback (b): a manual function-type factory `(String) -> DataStoreProvider` from a public `@BindingContainer @ContributesTo(AppScope)` (the lambda closes over the graph `Context`), no assisted framework at all. `Context` (create()-bound) and the constant `NAME` are the only inputs. Live consumers: `AppRootViewModel` + `feature/settings` (`SettingsInteractorImpl`, via `SettingsHiltEntryPoint`). **Residual risk:** dual-processor collision — `core/data/dataStore` does not apply Metro (§D10) but still runs Hilt-KSP (convention plugin), so adding Metro without removing the Hilt half of the convention would run both processors over the `@Module`/`@AssistedFactory`/`@AssistedInject`; resolved by opting the module off the Hilt convention at migration (as prior batch flips did). **Trigger to act:** its own execution slice — orthogonal to ImageStorage. |
| 🟡 test-infra, Step-5 | [core/core-android/.../images/ImageStorageImpl.kt](../core/core-android/src/main/kotlin/io/github/stslex/workeeper/core/core/images/ImageStorageImpl.kt) ↔ [TestInfraModule.kt](../core/ui/test-utils/src/main/kotlin/io/github/stslex/workeeper/core/ui/test/di/TestInfraModule.kt) | **`ImageStorage` — NOT assisted; blocked solely by the test-fake swap.** `ImageStorageImpl` is a plain `@Singleton @Inject constructor(@ApplicationContext Context, @IODispatcher CoroutineDispatcher)` bound by a plain Hilt `@Binds` — **zero `@Assisted`**. Both deps are already Metro-graph-available (`Context` create()-bound; `@IODispatcher` direct-provided), so structurally it is a **trivial D1 flip**. The **only** blocker is the test-fake swap: `TestInfraModule`'s `@TestInstallIn(replaces = [ImageStorageModule])` binds `FakeImageStorage` across the **15** suites that put `core:ui:test-utils` on their androidTest classpath (13 features + `core:ui:mvi` + `app/app` + `app/dev`; only the two app-scoped suites actually resolve it — the 13 feature `@Smoke` suites use hand-built State and never touch it). Today ImageStorage is a Hilt→Metro **forward bridge** (Hilt owns, Metro reads it as a `create()` bound instance), NOT a Metro→Hilt adopt-back, so the `AppGraphSourceModule`/`AdoptBack` honesty-split does **not** apply and the fake-swap is free + correct. Flipping it to Metro-owned would **delete** the `@TestInstallIn(replaces=[ImageStorageModule])` hook (no Hilt module left to replace), and Metro has **no `@TestInstallIn` analogue** — a test-override across 15 classpaths has no established repo pattern. **Fix path:** defer to **Step 5** and flip ImageStorage *together with* the DB-cascade substrate it is fenced alongside (the "C1 carveout"), designing the Metro test-override once for the whole cascade rather than bespoke for this binding. **Trigger to act:** Step-5 DB-cascade migration; coordinate with the feature androidTest app-scope debt fake-infra work. |
