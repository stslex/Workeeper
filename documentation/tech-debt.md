...
| Severity | Location | Description |
|---|---|---|
| 🟡 | [core/data/backup/scheduling/.../BackupPreferencesRepositoryImpl.kt:29](../core/data/backup/scheduling/src/main/kotlin/io/github/stslex/workeeper/core/data/backup/scheduling/BackupPreferencesRepositoryImpl.kt) | File `backup_scheduling_prefs`. **Reachable from `AppGraph` directly** — `AppGraph.kt:85` exposes `backupPreferencesRepository`. |
| 🟡 | [core/data/backup/scheduling/.../RestoreStateRepositoryImpl.kt:33](../core/data/backup/scheduling/src/main/kotlin/io/github/stslex/workeeper/core/data/backup/scheduling/RestoreStateRepositoryImpl.kt) | File `restore_state_prefs`. Not a direct `AppGraph` accessor; reached through the restore flow. |
| 🟡 | [feature/app-dialogs/impl/.../AppDialogRepository.kt:30](../feature/app-dialogs/impl/src/main/kotlin/io/github/stslex/workeeper/feature/app_dialogs/impl/data/AppDialogRepository.kt) | File `app_dialogs_prefs`. **Reachable from `AppGraph` directly** — `AppGraph.kt:96` exposes `appDialogRepository`. |
...
| Severity | Location | Description |
|---|---|---|
| 🟢 | [core/ui/kit/.../list/AppListRow.kt:52](../core/ui/kit/src/commonMain/kotlin/io/github/stslex/workeeper/core/ui/kit/components/list/AppListRow.kt) ↔ [feature/all-trainings/.../TrainingRow.kt:62](../feature/all-trainings/src/main/kotlin/io/github/stslex/workeeper/feature/all_trainings/ui/components/TrainingRow.kt) | **These two tags look like per-row selectors and cannot be used as one.** **Mechanism:** `AppListRow` applies `nameTestTag` / `metaTestTag` to the name and meta **`Text`s** (`AppListRow.kt:52` and `:123`), which are descendants of the inner `Row`. The click arrives on that `Row` via the `rowModifier` seam — `TrainingRow.kt:60` passes `combinedClickable(onClick, onLongClick)` — and `Modifier.clickable` merges descendant semantics, so the child's tag is **absent from the merged tree** that `onNodeWithTag` queries by default, while the node that actually carries the click action **has no tag of its own**. Net effect: the tag is unreachable for a click, and `useUnmergedTree = true` would find the `Text` but clicking it would not dispatch the row's handler. **Proof — measured, not reasoned:** selecting by tag times out on a row that is demonstrably on screen; the same row, same timing, responds to a text selector. **Current workaround:** `NavPaths.openTraining` clicks by unique seeded name and carries this explanation at its call site. **Unblock condition:** add a row-level `testTag` on the `rowModifier` chain (alongside `combinedClickable`, i.e. on the node that owns the click) — `AllTrainingsItemRow_<uuid>`. Exactly **one** call site changes: `NavPaths.openTraining`. The existing name/meta tags are still legitimate for asserting *text content*; they are simply not handles. **Deadline: none pinned (softened 2026-08-15)** — the original deadline said "before stage 1.3" because `StoreRetentionTest` and `BackStackStateRestorationTest` would need to open list rows; both shipped opening AllExercises rows (`AllExercisesItem_<uuid>` is a real row handle), so no new text-selector call site was added and `NavPaths.openTraining` remains the only one. Ripe for any hygiene PR. |

Two more tags in the same class, both read off a semantics dump rather than reasoned:

- **`LiveExerciseCardSub_<uuid>` is only in the unmerged tree.** It is tagged on a `Text` inside
  a live-exercise card that takes a `Modifier.clickable` to expand, so the whole card is one
  merged node and the tag is absent from the merged tree `onNodeWithTag` queries by default.
  `NavPaths.tagStartingWith` / `tagsStartingWith` / `assertUnmergedText` all need
  `useUnmergedTree = true` for it; without the flag the query times out on a node that is
  demonstrably on screen.
- **There is no usable `ExerciseEditScreen` tag** — it never reaches the tree. Both mode
  branches of the exercise graph receive the graph's `modifier`, which already carries
  `.testTag("ExerciseGraph")`, and a second `testTag` on the same chain does not add: in edit
  mode the tree carries `ExerciseGraph` plus the action bar's own tag and no
  `ExerciseEditScreen`. Edit mode's usable marker is **`ExerciseEditActionBar`**. (Older
  scenarios in `test-scenarios/exercise.md` still list `ExerciseEditScreen` as an expected tag.)
...
`AppCoroutineScopeImpl.kt:28-29` now assembles the scope as
