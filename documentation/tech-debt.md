...
| 🟢 | feature/exercise/, feature/single-training/ | **Exercise / SingleTraining still host AppPlanEditor as bottom sheets** (v2.4 D1 partial). The full-screen `Screen.PlanEditor` route is wired and live-workout migrated; Exercise detail and SingleTraining edit still use the deprecated `AppPlanEditor` sheet host plus per-feature `PlanEditActionHandler`. Migration is mechanical (mirror live-workout's `OpenPlanEditor` navigation + `SAVED_STATE_PLAN_EDITOR_SAVED` reload) but lives outside this stage's scope. |
