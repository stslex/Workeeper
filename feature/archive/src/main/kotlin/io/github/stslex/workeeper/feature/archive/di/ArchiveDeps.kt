 ...
 * copied VERBATIM from `AppGraphContract` so `AppGraph`'s existing overrides satisfy them with no new
 * provision. Types are owned by `core:data:exercise` / `core:core` (already depended on) — no new edge, no
