...
import io.github.stslex.workeeper.feature.plan_editor.resources.Res
...
fun NavGraphScope.planEditorGraph(
    ...
    navScreen<Screen.PlanEditor.Existing> { screen ->
        PlanEditorContent(
            modifier = modifier,
            processor = PlanEditorFeature(factory).processor(screen),
        )
    }
}
...
private fun PlanEditorContent(
    ...
    val loadFailedMessage = stringResource(Res.string.core_ui_plan_editor_error_load)
    val saveFailedMessage = stringResource(Res.string.core_ui_plan_editor_error_save)
