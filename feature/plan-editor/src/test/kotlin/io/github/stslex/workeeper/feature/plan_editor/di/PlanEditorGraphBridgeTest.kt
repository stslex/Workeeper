...
/**
 * KMP C.1 wave 3 — in-situ proof on plan-editor's REAL [PlanEditorGraph]. ASSISTED shape (5
 * handlers): the graph exposes the assisted Factory (never the Store); resolving it proves all 8
 * bridged singletons wired.
 */
internal class PlanEditorGraphBridgeTest {
    ...
    fun `assisted factory is exposed and resolvable from the real graph`() {
        val graph = createGraphFactory<PlanEditorGraph.Factory>()
            .create(
        ...
        assertNotNull(graph.storeFactory, "The assisted PlanEditorStoreImpl.Factory must be resolvable")
    }
}
